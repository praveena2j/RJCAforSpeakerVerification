...
class PositionAttn(nn.Module):
	...
	def __init__(self, embed_dim, dim):
		...
		self.affine_audio = nn.Linear(192, 192)
		self.affine_video = nn.Linear(512, 192)
		self.affine_v = nn.Linear(192, 49, bias=False)
		self.affine_g = nn.Linear(192, 49, bias=False)
		self.affine_h = nn.Linear(49, 1, bias=False)
		self.affine_feat = nn.Linear(512, 192)
		self.relu = nn.ReLU()
	...
	def forward(self, video, audio):
		...
		print(alpha_t.shape)
		print(V.shape)
		print(torch.bmm(alpha_t, V).shape)
		sys.exit()
